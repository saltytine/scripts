...
take_screenshot() {
    ...
    elif [[ "${store}" == "both" ]] ; then
        if [[ "${mode}" == "select" ]] ; then
            maim --hidecursor --select "${path_to_save}$(date +%s).png" | tee >(xclip -selection clipboard -t image/png >/dev/null)
        elif [[ "${mode}" == "full" ]] ; then
            maim --hidecursor "${path_to_save}$(date +%s).png" | tee >(xclip -selection clipboard -t image/png >/dev/null)
        fi
    fi
    send_notification
  exit 1
